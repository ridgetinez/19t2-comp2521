...
static void IntListUnlinkNode(IntList L, struct IntListNode *node)
{
	if (L->first == NULL) {
		return;
...
/* STATIC, has two meanings, in this context we mean it to say that
   this procedure can only be used in this file
*/
...
void IntListInsertInOrder(IntList L, int v)
{
	...
	if (L->first == NULL) {
		L->first = newIntListNode(v);
		L->last  = L->first;
		L->size++;
		return;
	}
	if (L->first->data >= v) {
		struct IntListNode *nextHead = newIntListNode(v);
		nextHead->next = L->first;
		L->first = nextHead;
		L->size++;
		return;
	}
	if (L->last->data <= v) {
		struct IntListNode *nextTail = newIntListNode(v);
		L->last->next = nextTail;
		L->last = nextTail;
		L->size++;
		return;
	}
	// our v is larger than our head
	struct IntListNode *curr = L->first;
	while (curr->next != NULL && curr->next->data < v) {
		curr = curr->next;
	}
	// we've stopped one node before the head
	struct IntListNode *nextNode = newIntListNode(v);
